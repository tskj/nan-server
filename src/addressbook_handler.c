...
void handle_get_request(header_t req) {
    ...
    sqlite3_bind_text(sql_statement, 1, id_to_get, -1, free);
    ...
    while ((rc = sqlite3_step(sql_statement)) == SQLITE_ROW) {
        ...
        tlf -> attributes = NULL;
        tlf -> nodes = NULL;
        tlf -> text = (char*) sqlite3_column_text(sql_statement, 2);
        ...
        name -> nodes = NULL;
        name -> text = (char*) sqlite3_column_text(sql_statement, 1);
        ...
        id -> attributes = NULL;
        id -> nodes = NULL;
        id -> text = (char*) sqlite3_column_text(sql_statement, 0);
